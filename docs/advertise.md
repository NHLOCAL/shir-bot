---
layout: advertise
title: "פרסום בשיר-בוט | הגיעו ל-14,000 לקוחות שהמתחרים שלכם מפספסים"
permalink: /advertise/
form_url: "https://docs.google.com/forms/d/e/1FAIpQLSeVvAKfHt5ZJA_fiGXgSFIp6CB1ZBBUZIbPZ38NdXUo-hBhmw/viewform?usp=dialog"
---

<!-- ======================================================= -->
<!-- HERO SECTION: CTA ממוקד תוצאה (שיפור ג'ואנה וויב)    -->
<!-- ======================================================= -->
<div class="hero-section">
    <h1>הגיעו ל-14,000 לקוחות שהמתחרים שלכם מפספסים.</h1>
    <p class="sub-headline">
        בשיר-בוט, המודעה שלכם לא נלחמת על תשומת לב. היא מקבלת במה מרכזית מול קהל שבוי של צעירים חרדים עם <strong>אינטרנט מסונן וטלפונים כשרים</strong> – קהל שאי אפשר להשיג בערוצים אחרים.
    </p>
    <div class="cta-button-wrapper">
        <a href="{{ page.form_url }}" target="_blank" rel="noopener" class="cta-button">
            אני רוצה להתחיל להגיע ללקוחות האלה
        </a>
    </div>
</div>

<!-- ======================================================= -->
<!-- STATS SECTION: מיקרו-קופי מחדד (שיפור אולי גרדנר)      -->
<!-- ======================================================= -->
<div class="stats-section">
    <h2>ההזדמנות שלכם במספרים</h2>
    <div class="stats-grid">
        <div class="stat-card">
            <div class="number">14,000</div>
            <div class="text">משתמשים ייחודיים בחודש</div>
        </div>
        <div class="stat-card">
            <div class="number">45,000+</div>
            <div class="text">חשיפות איכותיות למותג שלכם</div>
        </div>
        <div class="stat-card">
            <div class="number">11</div>
            <div class="text">כניסות בחודש לכל משתמש פעיל</div>
        </div>
    </div>
</div>

<!-- ======================================================= -->
<!-- PROBLEM/SOLUTION: ניסוח תוצאה חד (שיפור דונלד מילר) -->
<!-- ======================================================= -->
<div class="why-us-section">
    <h2>למה פרסום אצלנו עובד (ולמה במייל הכסף שלכם מתבזבז)</h2>
    <div class="problem-solution-grid">
        <div class="problem-card">
            <h3><i class="fas fa-trash-alt"></i> הבעיה עם פרסום במייל: המודעה שלכם הולכת לאיבוד</h3>
            <p>בניוזלטר, המודעה שלכם נלחמת על תשומת לב מול 5 מודעות אחרות. הלקוח סורק לשנייה, סוגר וממשיך הלאה. רוב הסיכויים שהוא בכלל לא זוכר שראה אתכם.</p>
        </div>
        <div class="solution-card">
            <h3><i class="fas fa-bullseye"></i> הפתרון בשיר-בוט: המודעה שלכם מקבלת 100% תשומת לב</h3>
            <p>כאן, המשתמש נמצא מרצונו, רגוע וממוקד. המודעה שלכם אינה הפרעה, אלא חלק מהחוויה שהוא מחפש. הוא פתוח למסר שלכם, ורואה אתכם שוב ושוב עד שהמותג שלכם הופך לבחירה הטבעית.</p>
        </div>
    </div>
</div>

<!-- ======================================================= -->
<!-- BENEFITS: תועלת ממוקדת יתרון תחרותי (שיפור סת' גודין)  -->
<!-- ======================================================= -->
<div class="benefits-grid-wrapper">
    <h2>3 תוצאות עסקיות ברורות מהפרסום אצלנו</h2>
    <div class="benefits-grid">
        <div class="benefit-card">
            <div class="icon"><i class="fas fa-sync-alt"></i></div>
            <h3>בנו אמון עם חשיפה יומיומית</h3>
            <p>הלקוחות שלכם חוזרים אלינו כל יום. הם יראו את המודעה שלכם שוב ושוב, עד שהמותג שלכם יהפוך לשם מוכר ומהימן שהם בוחרים בו באופן טבעי.</p>
        </div>
        <div class="benefit-card">
            <div class="icon"><i class="fas fa-shield-alt"></i></div>
            <h3>הרוויחו אמינות מיידית</h3>
            <p>הקהל שלכם סומך על שיר-בוט כסביבה נקייה ואיכותית. כשהעסק שלכם מופיע כאן, האמון הזה עובר אוטומטית גם אליכם.</p>
        </div>
        <div class="benefit-card">
            <div class="icon"><i class="fas fa-key"></i></div>
            <h3>הגיעו לקהל שהמתחרים לא יכולים</h3>
            <p>רוב המתחרים שלכם בכלל לא יודעים שהקהל הזה קיים, ואין להם שום דרך להגיע אליו. כאן אתם מקבלים גישה בלעדית וישירה ללקוחות שהם מפספסים לחלוטין.</p>
        </div>
    </div>
</div>

<!-- ======================================================= -->
<!-- SOCIAL PROOF (ללא שינוי, הפסקה חזקה)                  -->
<!-- ======================================================= -->
<div class="social-proof-section">
    <h2>המודעה שלכם תופיע בסביבה שהלקוחות שלכם אוהבים וסומכים עליה</h2>
    <p class="section-intro" style="max-width: 700px; margin: 0 auto 30px auto; color: #555;">הנה מה שהלקוחות העתידיים שלכם אומרים על המקום בו הם יפגשו את המותג שלכם:</p>
    <div class="testimonials-grid testimonials-grid-4">
        <blockquote class="testimonial">
            <p>"כל שיר שצריך <strong>תמיד</strong> היה בשיר בוט."</p>
            <footer>- ישראל כהן</footer>
        </blockquote>
        <blockquote class="testimonial">
            <p>"הרבה תוכן מדהים <strong>וכשר</strong>... לקוחה <strong>אסירת תודה!</strong>"</p>
            <footer>- לאה הורביץ</footer>
        </blockquote>
        <blockquote class="testimonial">
            <p>"אתר נהדר... <strong>מעודכן ויפהפה</strong>. מומלץ."</p>
            <footer>- חיימקה פלונצ'ק</footer>
        </blockquote>
        <blockquote class="testimonial">
            <p>"<strong>אין עליו בעולם!!!</strong>"</p>
            <footer>- רפאל ברוך טולדנו</footer>
        </blockquote>
    </div>
</div>

<!-- ======================================================= -->
<!-- PACKAGES: הסרת חסמים פסיכולוגיים (שיפור אולי גרדנר)    -->
<!-- ======================================================= -->
<div class="packages-section">
    <h2>בחרו את המסלול המתאים לעסק שלכם</h2>
    <div class="packages-grid">
        <div class="package-card">
            <h3><i class="fas fa-rocket"></i> חבילת הפתיחה החכמה</h3>
            <p class="package-for">הבטיחו שהעסק שלכם ייראה במקום הכי מרכזי באתר - בתוך רשימות השירים.</p>
            <p class="package-who"><strong>למי זה מתאים?</strong> לעסקים שרוצים לבדוק את העוצמה של שיר-בוט ולהפוך לשם מוכר בעלות של כוס קפה ליום.</p>
            <div class="price-box">
                <strong>190 ₪</strong> / לחודש
            </div>
            <ul>
                <li><i class="fas fa-check-circle text-success"></i><strong>מודעה משולבת ברשימת השירים</strong></li>
            </ul>
			<div class="package-visual">
				<img src="{{ site.baseurl }}/assets/images/ad-inline-example.png" alt="דוגמה למודעה משולבת ברשימת השירים" style="width:100%; border-radius: 6px;">
			</div>
            <a href="{{ page.form_url }}" target="_blank" rel="noopener" class="package-cta">אני מתחיל עם החבילה החכמה</a>
        </div>
        <div class="package-card package-card--recommended">
            <span class="package-badge">הבחירה הפופולרית</span>
            <h3><i class="fas fa-crown"></i> חבילת חשיפה מקסימלית</h3>
            <p class="package-for">הפכו למותג הדומיננטי באתר. הנוכחות שלכם בכל עמוד תבטיח שאף משתמש לא יפספס אתכם.</p>
            <p class="package-who"><strong>למי זה מתאים?</strong> לעסקים שרוצים להפוך לשם הכי מוכר בתחום, ומהר.</p>
            <div class="price-box">
                <strong>390 ₪</strong> / לחודש <br><small>במקום 550 ₪</small>
            </div>
            <ul>
                <li><i class="fas fa-check-circle text-success"></i><strong>באנר קבוע בכל עמודי האתר</strong></li>
                <li><i class="fas fa-plus-circle text-primary"></i><strong>בנוסף: מודעה משולבת ברשימות</strong></li>
            </ul>
			<div class="package-visual">
				<img src="{{ site.baseurl }}/assets/images/ad-banner-example.png" alt="דוגמה לבאנר פרסומי באתר שיר-בוט" style="width:100%; border-radius: 6px;">
			</div>
            <a href="{{ page.form_url }}" target="_blank" rel="noopener" class="package-cta">אני רוצה חשיפה מקסימלית</a>
        </div>
    </div>
</div>

<!-- ======================================================= -->
<!-- GUARANTEE: אחריות חסינת-כדורים (שיפור ג'ואנה וויב)   -->
<!-- ======================================================= -->
<div class="guarantee-section">
    <div class="guarantee-content">
        <i class="fas fa-handshake"></i>
        <h2>האחריות האישית שלנו: תאהבו את התוצאות, או שלא תשלמו שקל.</h2>
        <p>אנחנו כל כך בטוחים בערך שתקבלו, שהסיכון כולו עלינו. זה פשוט:</p>
        <ul class="guarantee-list">
            <li><i class="far fa-calendar-check"></i> <strong>בחרו ב"חבילת חשיפה מקסימלית"</strong> ופרסמו למשך חודש.</li>
            <li><i class="far fa-chart-bar"></i> <strong>עקבו אחר החשיפה</strong> והתגובות שתקבלו.</li>
            <li><i class="far fa-thumbs-down"></i> <strong>לא מרוצים מכל סיבה שהיא?</strong></li>
            <li><i class="fas fa-undo-alt"></i> <strong>קבלו החזר כספי מלא.</strong> שלחו מייל עם מילה אחת - "החזר" - וכספכם יוחזר במלואו, בלי שאלות ובלי אותיות קטנות.</li>
        </ul>
    </div>
</div>

<!-- ======================================================= -->
<!-- BONUS & FAQ (ללא שינוי מהותי)                          -->
<!-- ======================================================= -->
<div class="bonus-section">
    <h3><i class="fas fa-gift"></i> בונוס: אין לכם באנר? נעצב לכם אחד בחינם.</h3>
    <p>אל תתנו לעיצוב לעכב אתכם. שלחו לנו טקסט ולוגו, והצוות שלנו יכין עבורכם באנר מקצועי תוך 24 שעות – <strong>ללא עלות נוספת.</strong> המטרה שלנו היא שתתחילו לראות תוצאות, כמה שיותר מהר.</p>
</div>
<div class="faq-section">
    <h2>שאלות נפוצות</h2>
    <details>
        <summary>איך מתחילים ואיך משלמים?</summary>
        <div class="faq-answer">
            <p>ממלאים את טופס הפנייה הקצר, אנחנו חוזרים אליכם עם כל הפרטים, ולאחר אישור נשלח קישור מאובטח לתשלום. הפרסום שלכם יהיה באוויר תוך יום עסקים.</p>
        </div>
    </details>
    <details>
        <summary>למה זה יותר יעיל מפרסום במייל?</summary>
        <div class="faq-answer">
            <p>במייל אתם נלחמים על תשומת לב מול 5 מודעות אחרות. כאן, אתם מקבלים משתמש מרוכז, בלי תחרות על המסך, שרואה אתכם כחלק מהחוויה שהוא אוהב. זה ההבדל בין הפרעה להמלצה.</p>
        </div>
    </details>
     <details>
        <summary>האם המחירים יעלו?</summary>
        <div class="faq-answer">
            <p>כן. אלו מחירי השקה מיוחדים למפרסמים הראשונים. ככל שהביקוש יגדל, המחירים צפויים לעלות. הזמנה עכשיו נועלת לכם את המחיר הנוכחי ומבטיחה לכם את התנאים הטובים ביותר.</p>
        </div>
    </details>
</div>

<!-- ======================================================= -->
<!-- FINAL CTA: קריאה לפעולה אישית ודחופה                   -->
<!-- ======================================================= -->
<div class="cta-button-wrapper final-cta">
    <h2>שריינו את מקומכם במחיר השקה</h2>
    <p class="sub-headline">
       המחירים המיוחדים והאחריות המלאה מובטחים למספר מוגבל של מפרסמים. זו ההזדמנות שלכם להגיע לקהל ייחודי במחיר שלא יחזור.
    </p>
    <a href="{{ page.form_url }}" target="_blank" rel="noopener" class="cta-button">
        שריינו לי את המקום במחיר השקה
    </a>
</div>